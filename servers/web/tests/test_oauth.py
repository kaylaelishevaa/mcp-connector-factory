...
access gate (client_secret == WEB_MCP_TOKEN) is enforced.
"""
...
VALID_SECRET = "test-token-xyz123"  # matches conftest fixture (WEB_MCP_TOKEN)
...
class TestDCR:
    ...
    def test_register_returns_secret_NOT_equal_to_web_token(self, test_client):
...
class TestRedirectURIAllowlist:
    """Open-redirect mitigation — host must be claude.ai / claude.com /
    ...
    def test_accepts_claude_com(self, test_client):
        r = test_client.get(
            "/oauth/authorize",
            params={
                "client_id": "cb",
...
class TestToken:
    ...
    def test_token_supports_http_basic_auth(self, test_client):
        code = _get_code(test_client)
        basic = base64.b64encode(f"cb:{VALID_SECRET}".encode()).decode()
        r = test_client.post(
            "/oauth/token",
