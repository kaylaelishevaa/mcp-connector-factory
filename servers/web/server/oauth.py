...
Design summary:
- Access token issued by /oauth/token IS the WEB_MCP_TOKEN. Subsequent
  /mcp Bearer validation works without code changes.
- The actual access gate is /oauth/token requiring client_secret to equal
  WEB_MCP_TOKEN. This preserves the existing auth posture (caller must
...
- Authorize auto-approves (no user-consent UI). The tunnel URL is
  considered the access-control boundary alongside the secret.

Setup flow (Claude custom connector):
  Add custom connector →
    Name: Acme Property Web
    URL: https://web-mcp.example.com
    Advanced > OAuth Client ID: anything (e.g. "cb")
    Advanced > OAuth Client Secret: <WEB_MCP_TOKEN>
  Save → Claude does discovery + authorize + token → uses WEB_MCP_TOKEN
...
# an access_token unless they prove client_secret == WEB_MCP_TOKEN.
...
async def register_client(request: Request) -> JSONResponse:
    ...
    unless their client_secret equals WEB_MCP_TOKEN. The purpose of DCR
...
async def authorize(
    ...
    No user-consent UI — we trust whoever reaches this URL (the tunnel
    URL secrecy is the first access boundary). Real enforcement is at the
    token endpoint, which requires client_secret == WEB_MCP_TOKEN.
    """
...
async def token_endpoint(
    ...
    Access enforcement: client_secret MUST equal WEB_MCP_TOKEN.
    On success, access_token = WEB_MCP_TOKEN itself (one shared bearer).
    """
    ...
    # Real access gate: client_secret must equal WEB_MCP_TOKEN
