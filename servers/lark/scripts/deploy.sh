#!/usr/bin/env bash
#
# Lark MCP Server — deploy from local Mac → droplet.
#
# Usage:
...
echo "=== Deploying Lark MCP Server ==="
...
ssh "${REMOTE}" "cd ${PROJECT_DIR} && docker compose logs --tail=30 cloudflared | grep -E 'trycloudflare.com' || echo '(tunnel URL not yet shown, retry in 30s)'"
echo ""
echo "Health: ssh ${REMOTE} 'lark-mcp-tail'"
